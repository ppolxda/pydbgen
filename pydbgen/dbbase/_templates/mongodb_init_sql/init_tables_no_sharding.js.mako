...
db = db.getSiblingDB('${dbname}');
db.getCollection('${tname}').createIndex({ ${','.join(['"{}": {}'.format(i['name'], get_sort(i)) for i in iconfig['fields']]) } }, { 'background': true, 'unique': ${'true' if iconfig['db_options'].get('is_unique', 0) else 'false'} });
% endfor
